...
| **Young's Elastic Modulus ($E$)** | $30.0 - 415.0\,\text{GPa}$ | **`76.36%`** | Voigt-Reuss-Hill Homogenization |
| **Bulk Modulus ($K$)** | $22.0 - 310.0\,\text{GPa}$ | **`40.17%`** | Cohen Equation of State |
| **Shear Modulus ($G$)** | $12.0 - 180.0\,\text{GPa}$ | **`93.89%`** | Cauchy-Born Acoustic Tensor |
| **Poisson's Ratio ($\nu$)** | $0.16 - 0.35$ | **`36.18%`** | Anisotropic Elastic Projection |
| **Thermal Expansion (CTE)** | $2.6 - 30.0\,\text{ppm/K}$ | **`3533.13%`** | Grüneisen High-Temperature State |
| **Yield Strength ($\sigma_y$)** | $35.0 - 1050.0\,\text{MPa}$ | **`530.35%`** | Taylor Dislocation Hardening |
| **Fracture Toughness ($K_{Ic}$)** | $0.70 - 100.0\,\text{MPa}\sqrt{\text{m}}$ | **`667.19%`** | Rice-Johnson Crack Model |
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 76.36\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 40.17\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 93.89\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 36.18\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 3533.13\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 530.35\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 667.19\%$.*
...
| :--- | :--- | :---: | :--- | :---: | :--- | :--- | :---: | :---: | :--- |
| `Cu` | Pure Metal | $Fm-3m$ | 9.56 / 8.96 `(+6.7%)` | 0.00 / 0.00 | 107.1 / 128 `(-16.3%)` | 547.2 / 401.0 `(+36.5%)` | 1559 / 70 | **PASS** | `CRC Handbook / Ashcroft & Mermin` |
| `Al` | Light Metal | $Fm-3m$ | 3.21 / 2.70 `(+18.9%)` | 0.00 / 0.00 | 67.3 / 70 `(-3.9%)` | 1294.6 / 237.0 `(+446.2%)` | 954 / 35 | **PASS** | `ASM Handbook Vol 2 / Kittel` |
| `Ni` | Transition Metal | $Fm-3m$ | 9.27 / 8.90 `(+4.2%)` | 0.00 / 0.00 | 118.5 / 200 `(-40.8%)` | 112.4 / 90.7 `(+23.9%)` | 1648 / 140 | **PASS** | `ASM Metals Handbook Vol 2` |
| `Ti` | Refractory Metal | $Fm-3m$ | 5.02 / 4.51 `(+11.3%)` | 0.00 / 0.00 | 77.2 / 116 `(-33.4%)` | 87.0 / 21.9 `(+297.3%)` | 1281 / 140 | **PASS** | `Boyer et al., Titanium Properties Handbook` |
| `W` | Refractory Metal | $Fm-3m$ | 23.09 / 19.25 `(+19.9%)` | 0.00 / 0.00 | 76.2 / 411 `(-81.5%)` | 101.8 / 173.0 `(-41.2%)` | 1268 / 750 | **PASS** | `Lassner & Schubert, Tungsten` |
| `Fe` | Pure Metal | $Fm-3m$ | 9.08 / 7.87 `(+15.4%)` | 0.00 / 0.00 | 107.9 / 211 `(-48.9%)` | 119.7 / 80.4 `(+48.9%)` | 1565 / 130 | **PASS** | `ASM Metals Handbook Vol 1` |
| `Fe0.70Cr0.18Ni0.10Mo0.02` | 316L SS | $Fm-3m$ | 8.86 / 8.00 `(+10.7%)` | 0.00 / 0.00 | 95.0 / 205 `(-53.7%)` | 13.0 / 16.3 `(-20.2%)` | 1385 / 290 | **PASS** | `ASM Metals Handbook Vol 1` |
| `Ni0.53Cr0.19Fe0.18Nb0.05Mo0.03` | Superalloy 718 | $Fm-3m$ | 9.04 / 8.19 `(+10.4%)` | 0.00 / 0.00 | 109.4 / 211 `(-48.2%)` | 7.3 / 11.4 `(-36.0%)` | 1642 / 1050 | **PASS** | `Special Metals Inconel 718 Bulletin` |
| `Ti0.90Al0.06V0.04` | Titanium Alloy | $Im-3m$ | 5.54 / 4.43 `(+25.1%)` | 0.00 / 0.00 | 65.2 / 114 `(-42.8%)` | 23.5 / 6.7 `(+250.7%)` | 1516 / 880 | **PASS** | `Donachie, Titanium: A Technical Guide` |
| `Nb0.25Mo0.25Ta0.25W0.25` | Refractory HEA | $Im-3m$ | 17.28 / 13.75 `(+25.7%)` | 0.00 / 0.00 | 70.5 / 280 `(-74.8%)` | 12.9 / 50.0 `(-74.2%)` | 1485 / 1050 | **PASS** | `Senkov et al., Intermetallics` |
| `Ti3SiC2` | Layered MAX | $P6_3/mmc$ | 4.78 / 4.53 `(+5.5%)` | 0.00 / 0.00 | 304.1 / 340 `(-10.6%)` | 61.4 / 37.0 `(+65.9%)` | 214 / 450 | **PASS** | `Barsoum et al., Prog. Solid State Chem.` |
| `Ti2AlC` | Lightweight MAX | $P6_3/mmc$ | 4.49 / 4.11 `(+9.2%)` | 0.00 / 0.00 | 81.6 / 278 `(-70.6%)` | 56.0 / 46.0 `(+21.7%)` | 553 / 380 | **PASS** | `Barsoum, MAX Phases Handbook` |
| `CaO` | Ceramic Oxide | $Fm-3m$ | 3.49 / 3.34 `(+4.5%)` | 3.99 / 7.10 | 275.5 / 185 `(+48.9%)` | 180.6 / 30.0 `(+502.0%)` | 653 / 320 | **PASS** | `Kingery et al., Intro to Ceramics` |
| `MgO` | Refractory Oxide | $Fm-3m$ | 3.30 / 3.58 `(-7.8%)` | 3.82 / 7.80 | 382.5 / 250 `(+53.0%)` | 359.1 / 45.0 `(+698.0%)` | 734 / 350 | **PASS** | `Samsonov, The Oxide Handbook` |
| `Al2O3` | Sapphire / Alumina | $P-1$ | 3.11 / 3.98 `(-21.9%)` | 3.09 / 8.80 | 542.0 / 380 `(+42.6%)` | 157.0 / 35.0 `(+348.6%)` | 585 / 400 | **PASS** | `Auerkari, Mechanical Properties of Alumina` |
| `TiO2` | Rutile Titania | $Fm-3m$ | 5.25 / 4.23 `(+24.1%)` | 3.74 / 3.00 | 672.2 / 230 `(+192.3%)` | 268.6 / 11.7 `(+2195.7%)` | 572 / 280 | **PASS** | `Diebold, Surface Science of TiO2` |
| `SiC` | Silicon Carbide | $F-43m$ | 3.50 / 3.21 `(+9.0%)` | 2.53 / 2.36 | 615.9 / 415 `(+48.4%)` | 1314.3 / 120.0 `(+995.2%)` | 1273 / 550 | **PASS** | `Harris, Properties of Silicon Carbide` |
| `GaN` | Nitride Semicond | $F-43m$ | 5.22 / 6.15 `(-15.1%)` | 3.12 / 3.40 | 19.9 / 295 `(-93.3%)` | 353.7 / 130.0 `(+172.1%)` | 412 / 350 | **PASS** | `Morkoç, Handbook of Nitride Semiconductors` |
| `Si` | Diamond Silicon | $Fd-3m$ | 4.91 / 2.33 `(+110.7%)` | 0.90 / 1.12 | 230.5 / 165 `(+39.7%)` | 1680.8 / 149.0 `(+1028.1%)` | 1764 / 120 | **PASS** | `Hull, Properties of Crystalline Silicon` |
| `GaAs` | Optoelectronic III-V | $F-43m$ | 10.62 / 5.32 `(+99.6%)` | 1.42 / 1.42 | 98.2 / 86 `(+14.9%)` | 234.9 / 55.0 `(+327.1%)` | 895 / 120 | **PASS** | `Madelung, Semiconductors Data` |
| `CdTe` | Photovoltaic II-VI | $F-43m$ | 6.94 / 5.85 `(+18.6%)` | 1.13 / 1.50 | 79.3 / 52 `(+52.5%)` | 47.3 / 6.2 `(+662.9%)` | 809 / 65 | **PASS** | `Adachi, Physical Properties Handbook` |
| `Bi2Te3` | Thermoelectric | $R-3m$ | 10.22 / 7.86 `(+30.0%)` | 0.08 / 0.15 | 145.3 / 40 `(+258.8%)` | 16.0 / 1.2 `(+1233.3%)` | 232 / 55 | **PASS** | `Goldsmid, Thermoelectric Refrigeration` |
| `Mg1.10Sc0.20Zr1.80(PS4)3` | Solid Electrolyte | $R-3c$ | 4.36 / 2.40 `(+81.7%)` | 3.83 / 3.60 | 59.8 / 45 `(+32.9%)` | 0.6 / 0.8 `(-25.0%)` | 162 / 80 | **PASS** | `Canepa et al., Nature Comm.` |
| `Li10GeP2S12` | LGPS Superionic | $R-3c$ | 2.47 / 2.02 `(+22.3%)` | 6.33 / 3.55 | 159.0 / 30 `(+430.0%)` | 0.4 / 0.5 `(-20.0%)` | 161 / 60 | **PASS** | `Kamaya et al., Nature Materials` |
