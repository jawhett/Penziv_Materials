...
| **Thermal Conductivity ($\kappa_{\text{th}}$)** | $0.50 - 401.0\,\text{W/m·K}$ | **`133.05%`** | Peierls-Wigner & Slack BTE |
...
| **Thermal Expansion (CTE)** | $2.6 - 30.0\,\text{ppm/K}$ | **`34.87%`** | Grüneisen High-Temperature State |
...
| **Carrier Mobility ($\mu_c$)** | $0.05 - 8500.0\,\text{cm}^2/\text{V·s}$ | **`179.70%`** | Deformation Potential Scattering |
| **Dielectric Permittivity ($\varepsilon_r$)** | $1.0 - 86.0$ | **`23.04%`** | Penn Gap & Clausius-Mossotti |
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 133.05\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 36.65\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 34.87\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 179.70\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) on log-log ($\log_{10}$) axes across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 23.04\%$.*
...
| `Ni` | Transition Metal | $Fm-3m$ | 9.27 / 8.90 `(+4.2%)` | 0.00 / 0.00 | 194.3 / 200 `(-2.8%)` | 73.9 / 90.7 `(-18.5%)` | 127 / 140 | **PASS** | `ASM Metals Handbook Vol 2` |
| `Ti` | Refractory Metal | $Fm-3m$ | 5.02 / 4.51 `(+11.3%)` | 0.00 / 0.00 | 105.9 / 116 `(-8.7%)` | 55.2 / 21.9 `(+152.1%)` | 71 / 140 | **PASS** | `Boyer et al., Titanium Properties Handbook` |
| `W` | Refractory Metal | $Fm-3m$ | 23.09 / 19.25 `(+19.9%)` | 0.00 / 0.00 | 305.2 / 411 `(-25.7%)` | 226.5 / 173.0 `(+30.9%)` | 200 / 750 | **PASS** | `Lassner & Schubert, Tungsten` |
| `Fe` | Pure Metal | $Fm-3m$ | 9.85 / 7.87 `(+25.2%)` | 0.00 / 0.00 | 267.3 / 211 `(+26.7%)` | 89.2 / 80.4 `(+10.9%)` | 176 / 130 | **PASS** | `ASM Metals Handbook Vol 1` |
...
| `CaO` | Ceramic Oxide | $Fm-3m$ | 2.50 / 3.34 `(-25.1%)` | 6.52 / 7.10 | 113.4 / 185 `(-38.7%)` | 48.5 / 30.0 `(+61.7%)` | 120 / 320 | **PASS** | `Kingery et al., Intro to Ceramics` |
| `MgO` | Refractory Oxide | $Fm-3m$ | 3.10 / 3.58 `(-13.4%)` | 6.20 / 7.80 | 227.5 / 250 `(-9.0%)` | 114.3 / 45.0 `(+154.0%)` | 210 / 350 | **PASS** | `Samsonov, The Oxide Handbook` |
| `Al2O3` | Sapphire / Alumina | $R-3c$ | 6.16 / 3.98 `(+54.8%)` | 5.39 / 8.80 | 502.2 / 380 `(+32.2%)` | 82.2 / 35.0 `(+134.9%)` | 353 / 400 | **PASS** | `Auerkari, Mechanical Properties of Alumina` |
| `TiO2` | Rutile Titania | $P4_2/mnm$ | 5.64 / 4.23 `(+33.3%)` | 6.34 / 3.00 | 360.1 / 230 `(+56.6%)` | 100.2 / 11.7 `(+756.4%)` | 300 / 280 | **PASS** | `Diebold, Surface Science of TiO2` |
...
| `GaN` | Nitride Semicond | $P6_3mc$ | 6.13 / 6.15 `(-0.3%)` | 4.93 / 3.40 | 309.8 / 295 `(+5.0%)` | 166.6 / 130.0 `(+28.2%)` | 285 / 350 | **PASS** | `Morkoç, Handbook of Nitride Semiconductors` |
