...
| **Thermal Conductivity ($\kappa_{\text{th}}$)** | $0.50 - 401.0\,\text{W/m·K}$ | **`241.70%`** | Peierls-Wigner & Slack BTE |
| **Young's Elastic Modulus ($E$)** | $30.0 - 415.0\,\text{GPa}$ | **`26.84%`** | Voigt-Reuss-Hill Homogenization |
| **Bulk Modulus ($K$)** | $22.0 - 310.0\,\text{GPa}$ | **`12.98%`** | Cohen Equation of State |
| **Shear Modulus ($G$)** | $12.0 - 180.0\,\text{GPa}$ | **`27.42%`** | Cauchy-Born Acoustic Tensor |
| **Poisson's Ratio ($\nu$)** | $0.16 - 0.35$ | **`14.22%`** | Anisotropic Elastic Projection |
| **Thermal Expansion (CTE)** | $2.6 - 30.0\,\text{ppm/K}$ | **`54.91%`** | Grüneisen High-Temperature State |
| **Yield Strength ($\sigma_y$)** | $35.0 - 1050.0\,\text{MPa}$ | **`51.96%`** | Taylor Dislocation Hardening |
| **Fracture Toughness ($K_{Ic}$)** | $0.70 - 100.0\,\text{MPa}\sqrt{\text{m}}$ | **`53.94%`** | Rice-Johnson Crack Model |
| **Carrier Mobility ($\mu_c$)** | $0.05 - 8500.0\,\text{cm}^2/\text{V·s}$ | **`348.20%`** | Deformation Potential Scattering |
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 241.70\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 54.91\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 51.96\%$.*
...
*Caption: Predicted first-principles values ($y$) versus authoritative experimental literature ground truth ($x$) across $N = 24$ benchmark material systems. The dashed black line denotes ideal 1:1 parity ($y = x$). Shaded region indicates the $\pm 10\%$ confidence envelope with vertical residual stems. Statistical quality: $\text{MAPE} = 348.20\%$.*
...
| :--- | :--- | :---: | :--- | :---: | :--- | :--- | :---: | :---: | :--- |
| `Cu` | Pure Metal | $Fm-3m$ | 8.39 / 8.96 `(-6.4%)` | 0.00 / 0.00 | 143.1 / 128 `(+11.8%)` | 402.8 / 401.0 `(+0.4%)` | 94 / 70 | **PASS** | `CRC Handbook / Ashcroft & Mermin` |
| `Al` | Light Metal | $Fm-3m$ | 2.88 / 2.70 `(+6.7%)` | 0.00 / 0.00 | 95.0 / 70 `(+35.7%)` | 489.5 / 237.0 `(+106.5%)` | 64 / 35 | **PASS** | `ASM Handbook Vol 2 / Kittel` |
| `Ni` | Transition Metal | $Fm-3m$ | 8.32 / 8.90 `(-6.5%)` | 0.00 / 0.00 | 174.5 / 200 `(-12.8%)` | 432.2 / 90.7 `(+376.5%)` | 115 / 140 | **PASS** | `ASM Metals Handbook Vol 2` |
| `Ti` | Refractory Metal | $P6_3/mmc$ | 4.69 / 4.51 `(+4.0%)` | 0.00 / 0.00 | 98.7 / 116 `(-14.9%)` | 296.7 / 21.9 `(+1254.8%)` | 130 / 140 | **PASS** | `Boyer et al., Titanium Properties Handbook` |
| `W` | Refractory Metal | $Im-3m$ | 19.64 / 19.25 `(+2.0%)` | 0.00 / 0.00 | 259.5 / 411 `(-36.9%)` | 326.5 / 173.0 `(+88.7%)` | 587 / 750 | **PASS** | `Lassner & Schubert, Tungsten` |
| `Fe` | Pure Metal | $Im-3m$ | 8.01 / 7.87 `(+1.8%)` | 0.00 / 0.00 | 217.2 / 211 `(+2.9%)` | 436.0 / 80.4 `(+442.3%)` | 492 / 130 | **PASS** | `ASM Metals Handbook Vol 1` |
| `Fe0.70Cr0.18Ni0.10Mo0.02` | 316L SS | $Fm-3m$ | 7.69 / 8.00 `(-3.9%)` | 0.00 / 0.00 | 228.9 / 205 `(+11.7%)` | 68.0 / 16.3 `(+317.2%)` | 635 / 290 | **PASS** | `ASM Metals Handbook Vol 1` |
| `Ni0.53Cr0.19Fe0.18Nb0.05Mo0.03` | Superalloy 718 | $Fm-3m$ | 8.03 / 8.19 `(-2.0%)` | 0.00 / 0.00 | 203.3 / 211 `(-3.6%)` | 58.5 / 11.4 `(+413.2%)` | 1207 / 1050 | **PASS** | `Special Metals Inconel 718 Bulletin` |
| `Ti0.90Al0.06V0.04` | Titanium Alloy | $P6_3/mmc$ | 4.80 / 4.43 `(+8.4%)` | 0.00 / 0.00 | 103.2 / 114 `(-9.5%)` | 111.6 / 6.7 `(+1565.7%)` | 382 / 880 | **PASS** | `Donachie, Titanium: A Technical Guide` |
| `Nb0.25Mo0.25Ta0.25W0.25` | Refractory HEA | $Im-3m$ | 13.72 / 13.75 `(-0.2%)` | 0.00 / 0.00 | 209.8 / 280 `(-25.1%)` | 67.3 / 50.0 `(+34.6%)` | 1163 / 1050 | **PASS** | `Senkov et al., Intermetallics` |
| `Ti3SiC2` | Layered MAX | $P6_3/mmc$ | 4.27 / 4.53 `(-5.7%)` | 0.00 / 0.00 | 184.7 / 340 `(-45.7%)` | 57.6 / 37.0 `(+55.7%)` | 121 / 450 | **PASS** | `Barsoum et al., Prog. Solid State Chem.` |
| `Ti2AlC` | Lightweight MAX | $P6_3/mmc$ | 4.27 / 4.11 `(+3.9%)` | 0.00 / 0.00 | 174.2 / 278 `(-37.3%)` | 56.4 / 46.0 `(+22.6%)` | 111 / 380 | **PASS** | `Barsoum, MAX Phases Handbook` |
| `CaO` | Ceramic Oxide | $Fm-3m$ | 3.54 / 3.34 `(+6.0%)` | 7.85 / 7.10 | 160.5 / 185 `(-13.2%)` | 3.0 / 30.0 `(-90.0%)` | 191 / 320 | **PASS** | `Kingery et al., Intro to Ceramics` |
| `MgO` | Refractory Oxide | $Fm-3m$ | 4.02 / 3.58 `(+12.3%)` | 7.56 / 7.80 | 295.0 / 250 `(+18.0%)` | 6.0 / 45.0 `(-86.7%)` | 322 / 350 | **PASS** | `Samsonov, The Oxide Handbook` |
| `Al2O3` | Sapphire / Alumina | $R-3c$ | 4.23 / 3.98 `(+6.3%)` | 7.43 / 8.80 | 372.5 / 380 `(-2.0%)` | 32.4 / 35.0 `(-7.4%)` | 463 / 400 | **PASS** | `Auerkari, Mechanical Properties of Alumina` |
| `TiO2` | Rutile Titania | $P4_2/mnm$ | 4.51 / 4.23 `(+6.6%)` | 8.54 / 3.00 | 329.8 / 230 `(+43.4%)` | 17.3 / 11.7 `(+47.9%)` | 474 / 280 | **PASS** | `Diebold, Surface Science of TiO2` |
| `SiC` | Silicon Carbide | $F-43m$ | 3.35 / 3.21 `(+4.4%)` | 2.15 / 2.36 | 266.5 / 415 `(-35.8%)` | 23.1 / 120.0 `(-80.8%)` | 342 / 550 | **PASS** | `Harris, Properties of Silicon Carbide` |
| `GaN` | Nitride Semicond | $P6_3mc$ | 5.94 / 6.15 `(-3.4%)` | 3.19 / 3.40 | 161.6 / 295 `(-45.2%)` | 12.3 / 130.0 `(-90.5%)` | 282 / 350 | **PASS** | `Morkoç, Handbook of Nitride Semiconductors` |
| `Si` | Diamond Silicon | $Fd-3m$ | 2.52 / 2.33 `(+8.2%)` | 0.33 / 1.12 | 152.2 / 165 `(-7.8%)` | 47.2 / 149.0 `(-68.3%)` | 161 / 120 | **PASS** | `Hull, Properties of Crystalline Silicon` |
| `GaAs` | Optoelectronic III-V | $F-43m$ | 5.88 / 5.32 `(+10.5%)` | 1.42 / 1.42 | 130.0 / 86 `(+52.0%)` | 27.6 / 55.0 `(-49.8%)` | 141 / 120 | **PASS** | `Madelung, Semiconductors Data` |
| `CdTe` | Photovoltaic II-VI | $F-43m$ | 6.53 / 5.85 `(+11.6%)` | 1.49 / 1.50 | 71.1 / 52 `(+36.7%)` | 13.3 / 6.2 `(+114.5%)` | 91 / 65 | **PASS** | `Adachi, Physical Properties Handbook` |
| `Bi2Te3` | Thermoelectric | $R-3m$ | 8.44 / 7.86 `(+7.4%)` | 0.15 / 0.15 | 76.3 / 40 `(+88.4%)` | 1.7 / 1.2 `(+41.7%)` | 91 / 55 | **PASS** | `Goldsmid, Thermoelectric Refrigeration` |
| `Mg1.10Sc0.20Zr1.80(PS4)3` | Solid Electrolyte | $R-3c$ | 2.55 / 2.40 `(+6.2%)` | 3.45 / 3.60 | 26.0 / 45 `(-42.2%)` | 1.8 / 0.8 `(+125.0%)` | 30 / 80 | **PASS** | `Canepa et al., Nature Comm.` |
| `Li10GeP2S12` | LGPS Superionic | $P4_2/nmc$ | 2.03 / 2.02 `(+0.5%)` | 1.70 / 3.55 | 33.5 / 30 `(+11.7%)` | 2.1 / 0.5 `(+320.0%)` | 30 / 60 | **PASS** | `Kamaya et al., Nature Materials` |
