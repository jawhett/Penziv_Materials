...
class AtomDynAgent:
    ...
    def execute_atomistic_evaluation(
        ...
        # Fundamental statistical variance in local migration barrier from chemical disorder:
        # sigma_Ea^2 = sum_i x_i * (E_i - <E>)^2
        from penziv_materials.scale5_quantum.q_elec import UniversalElementalProperties
        ...
        mean_b = max(0.1, sum((c / tot_cnt) * b for c, b in zip(c_cnts, elem_barriers)))
        chem_var = sum((c / tot_cnt) * ((b - mean_b) ** 2) for c, b in zip(c_cnts, elem_barriers))
        force_variance = float(np.sqrt(chem_var))

        # Authentic transition state theory kinetic rate variance across alloy barrier distribution:
        # sigma_ln_gamma^2 = sigma_Ea^2 / <E_a>^2
        sigma_ln_gamma_sq = float(round(chem_var / (mean_b ** 2), 4))
